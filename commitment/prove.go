...
func (proof LocalProof) BatchVerifyLocalProof(
	...
	for j := 0; j < num_commit; j++ {
		...
		//fmt.Println("ti_s[j]", hex.EncodeToString(ti_s[j][0].Serialize()))
	}
	fmt.Println(" 1111ti_s = [")
		for i, sub_ti_s := range ti_s {
			fmt.Print("  [")
			for j, value := range sub_ti_s {
				if j > 0 {
					fmt.Print(", ")
				}
				fmt.Printf("%s", hex.EncodeToString(value.Serialize()))
			}
			fmt.Printf("],  // ti_s\n", i)
		}
		fmt.Println("]")
	...
	fmt.Println("ti0", hex.EncodeToString(ti[0].Serialize()))
	fmt.Println("ti1", hex.EncodeToString(ti[1].Serialize()))
	fmt.Println("ti2", hex.EncodeToString(ti[2].Serialize()))
